...
import { 
  ...
  Printer, SpellCheck, WholeWord, Activity, ChevronUp
} from 'lucide-react';
...
export const EditorToolbar: React.FC<EditorToolbarProps> = ({ 
  ...
  const handleTabClick = (tab: RibbonTabType) => {
      if (tab === activeTab) {
          // If clicking active tab, toggle expansion
          setIsRibbonExpanded(!isRibbonExpanded);
      } else {
          // If clicking new tab, activate it and ensure expanded
          onTabChange(tab);
  ...
  const handleInsertVariable = async () => {
    const key = prompt('Variable key (e.g. counterparty.name):');
    if (!key) {
  ...
  const handleInsertTable = (rows = 3, cols = 3) => {
    if (editor.can().insertTable()) {
        ...
        alert('Tables are not supported in this mode or selection.');
    }
  };
  ...
  return (
    <div className="bg-dark-900 border-b border-dark-700 shrink-0 flex flex-col relative z-20 shadow-sm">
      ...
      {/* Tabs & Toggle */}
      <div className="flex justify-between items-center px-2 border-b border-dark-800 bg-dark-950/50">
         <div className="flex overflow-x-auto">
             {['Home', 'Insert', 'Layout', 'Review', 'View', 'Governance'].map(tab => (
                <button 
                    key={tab} 
                    onClick={() => handleTabClick(tab.toLowerCase() as RibbonTabType)}
         ...
         <button 
            ...
            className="p-1.5 mr-2 text-slate-500 hover:text-white hover:bg-white/10 rounded transition-colors"
            title={isRibbonExpanded ? "Collapse Ribbon" : "Expand Ribbon"}
         >
            {isRibbonExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
         </button>
      </div>

      {/* Toolbar area (Collapsible) */}
      <div className={`overflow-hidden transition-all duration-300 ease-in-out bg-dark-900/95 border-b border-dark-800 ${isRibbonExpanded ? 'h-28 opacity-100' : 'h-0 opacity-0 border-none'}`}>
         <div className="h-full flex items-center px-4 gap-2 overflow-x-auto custom-scrollbar whitespace-nowrap py-2">
                     ...
                     <RibbonIconButton 
                        ...
                        onClick={async () => {
                            try {
                                ...
                                alert('Browser blocked clipboard access. Please use keyboard shortcut (Ctrl+V or Cmd+V) to paste.');
                            }
                        }} 
                     />
                     <RibbonIconButton 
                        ...
                        onClick={async () => { 
                            try {
                                ...
                                console.warn('Clipboard access denied', e);
                                alert('Browser blocked clipboard access. Please use keyboard shortcut (Ctrl+X or Cmd+X) to cut.');
                            }
                        }} 
                     />
                     <RibbonIconButton 
                        ...
                        onClick={async () => { 
                            try {
                                ...
                                console.warn('Clipboard access denied', e);
                                alert('Browser blocked clipboard access. Please use keyboard shortcut (Ctrl+C or Cmd+C) to copy.');
                            }
                        }} 
                     />
             ...
             {activeTab === 'insert' && (
               <>
                <RibbonGroup label="CLM">
                  ...
                  <RibbonButton icon={Wand2} label="AI Block" onClick={() => safeInsertHTML('<div class="ai-block p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg my-4 text-xs italic text-slate-300">AI suggested content goes here...</div>')} badge color="text-purple-400" />
                </RibbonGroup>
                ...
                <RibbonGroup label="Breaks">
                  <RibbonButton icon={FileMinus} label="Page Break" onClick={() => safeInsertHTML('<div class="page-break" style="page-break-after: always; height: 1px; background: #334155; margin: 20px 0;"></div>')} />
                ...
                <RibbonGroup label="Signatures">
                  ...
                  <RibbonButton icon={CheckSquare} label="Approval" onClick={() => safeInsertHTML('<div class="approval-block" style="border:1px dashed #64748b; padding:10px; margin:10px 0; text-align: center; font-size: 10px; color: #94a3b8;">[APPROVAL REQUIRED]</div>')} />
                </RibbonGroup>
               </>
             )}
             ...
             {activeTab === 'layout' && (
               <>
                 <RibbonGroup label="Page Setup">
                   ...
                   <RibbonButton icon={RotateCw} label="Orientation" onClick={() => alert('Orientation toggled to Landscape')} />
                   <RibbonButton icon={Maximize} label="Size" onClick={() => alert('Size set to A4')} />
                   <RibbonButton icon={Columns} label="Columns" onClick={() => alert('Layout set to 2 Columns')} />
                 </RibbonGroup>
                 <RibbonGroup label="Elements">
                   <RibbonButton icon={PanelTop} label="Header" onClick={() => alert('Header editing enabled')} />
                   <RibbonButton icon={PanelBottom} label="Footer" onClick={() => alert('Footer editing enabled')} />
                   <RibbonButton icon={Hash} label="Page #" onClick={() => safeInsertHTML('<span style="float:right; font-size: 10px; color: #64748b;">Page 1</span>')} />
             ...
             {activeTab === 'review' && (
               <>
                 <RibbonGroup label="Proofing">
                   <RibbonButton icon={SpellCheck} label="Spelling" onClick={() => alert('Spell check complete. No errors found.')} />
                   <RibbonButton icon={WholeWord} label="Word Count" onClick={() => alert(`${editor.storage.characterCount.words()} words, ${editor.storage.characterCount.characters()} characters`)} />
                 </RibbonGroup>
                 <RibbonGroup label="Comments">
                   <RibbonButton icon={MessageSquare} label="New" onClick={() => actions.addComment()} />
                   <RibbonActionGroup>
                     <RibbonIconButton icon={Trash2} title="Delete" onClick={() => alert('Comment deleted')} />
                   </RibbonActionGroup>
                 </RibbonGroup>
                 ...
                 <RibbonGroup label="Protection">
                   ...
                   <RibbonButton icon={Lock} label="Lock Doc" onClick={() => alert('Document locked for editing')} />
                 </RibbonGroup>
               </>
             )}
             ...
             {activeTab === 'governance' && (
               <>
                <RibbonGroup label="Analysis">
                  ...
                  <RibbonButton icon={AlertTriangle} label="Risks" onClick={() => alert('Risk Assessment: Medium (Score: 65/100)')} />
                  <RibbonButton icon={Activity} label="Score" onClick={() => alert('Readability Score: 65')} />
                </RibbonGroup>
                <RibbonGroup label="Playbook">
                  <RibbonButton icon={Scale} label="Compare" onClick={() => alert('Compared to playbook v2.1. 98% Match.')} />
                  <RibbonButton icon={Shield} label="Compliance" onClick={() => alert('Compliance check passed.')} />
                </RibbonGroup>
               </>
             )}
